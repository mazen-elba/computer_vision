...
namespace sr = social_relation_msgs;
    ...
    for (int i = 0; i < socialActivityVisual->typeTexts.size(); i++)
    {
        ...
        { // might be not set if center of activity could not be determined
            ...
            if (m_activity_type_per_track_property->getBool())
            {
                ...
                centerAt = Ogre::Vector3(socialActivityVisual->socialActivityCenter.x, socialActivityVisual->socialActivityCenter.y, socialActivityVisual->socialActivityCenter.z);
            ...
            typeText->setPosition(m_frameTransform * Ogre::Vector3(
                                                         centerAt.x,
                                                         centerAt.y,
                                                         centerAt.z + m_activity_type_offset->getFloat() + m_commonProperties->z_offset->getFloat() + socialActivityVisual->declutteringOffset * characterHeight /* this is for decluttering of overlapping labels */));
        }
    }
}
...
void SocialActivitiesDisplay::processMessage(const social_relation_msgs::SocialActivities::ConstPtr &msg)
{
    ...
    //
    foreach (const social_relation_msgs::SocialActivity &socialActivity, msg->elements)
    {
        ...
        for (size_t trackIndex = 0; trackIndex < socialActivity.track_ids.size(); trackIndex++)
        {
            ...
            if (m_highestConfidenceActivityPerTrack.find(trackId) == m_highestConfidenceActivityPerTrack.end() || socialActivity.confidence > m_highestConfidenceActivityPerTrack[trackId].confidence)
            {
    ...
    {
        const social_relation_msgs::SocialActivity &socialActivity = msg->elements[i];
...
PLUGINLIB_EXPORT_CLASS(tracking_rviz_plugin::SocialActivitiesDisplay, rviz::Display)

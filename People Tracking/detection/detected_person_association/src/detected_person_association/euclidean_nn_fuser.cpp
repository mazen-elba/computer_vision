...
{
float EuclideanNNFuserNodelet::computeDistance(const tracking_msgs::CompositeDetectedPerson &d1, const tracking_msgs::CompositeDetectedPerson &d2)
{
    ...
    double gatingDistance = 0.5;
    getPrivateNodeHandle().getParamCached("gating_distance", gatingDistance);
    if (distance > gatingDistance)
        distance = std::numeric_limits<float>::infinity();

    return distance;
}

void EuclideanNNFuserNodelet::fusePoses(const tracking_msgs::CompositeDetectedPerson &d1, const tracking_msgs::CompositeDetectedPerson &d2, geometry_msgs::PoseWithCovariance &fusedPose)
{
    // Obtain and normalize weights of arithmetic mean
    double weight1 = 0.5;
    getPrivateNodeHandle().getParamCached("pose_weight_for_topic1", weight1);
    double weight2 = 0.5;
    getPrivateNodeHandle().getParamCached("pose_weight_for_topic2", weight2);
...
PLUGINLIB_DECLARE_CLASS(detected_person_association, EuclideanNNFuserNodelet, detected_person_association::EuclideanNNFuserNodelet, nodelet::Nodelet)

...
#include <tracking_msgs/TrackedPersons.h>
...
protected:
    ...
    bool hasValidOrientation(const geometry_msgs::PoseWithCovariance &pose)
    {
        ...
        if (pose.pose.orientation.x == 0 && pose.pose.orientation.y == 0 && pose.pose.orientation.z == 0 && pose.pose.orientation.w == 0)
            return false;
    ...
    Ogre::Matrix3 covarianceXYZIntoTargetFrame(const geometry_msgs::PoseWithCovariance &pose)
    {
        ...
        if (!isfinite(xyzcov.Determinant()))
            ROS_WARN_STREAM("Covariance matrix supplied to covarianceXYZIntoTargetFrame() contains non-finite elements: " << xyzcov);

...
bool eval(string result_sha, Mail *mail)
{
    ...
    if (eval_disp)
        mail->msg("Evaluating stereo results.");
    if (eval_flow)
        mail->msg("Evaluating flow results.");
    if (eval_scene_flow)
        mail->msg("Evaluating scene flow results.");
...
int32_t main(int32_t argc, char *argv[])
{
    ...
    if (argc == 4)
        mail->finalize(success, "scene_flow", result_sha, argv[2]);
    else
        mail->finalize(success, "scene_flow", result_sha);

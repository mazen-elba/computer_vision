...
class FpnAnchorGeneratorTest(tf.test.TestCase):
    def test_construct_pyramid(self):
        ...
        with self.test_session():
            ...
            expected_num_anchors = sum(
                [h * w * 3 for h, w in feature_map_shape_list])
            ...
            for i, grid_size, scale in zip(range(5), feature_map_shape_list, scales):
                ...
                this_anchors_out = anchors_out[prev_grid_elems:
                                               this_grid_elems + prev_grid_elems]
                this_areas_out = areas_out[prev_grid_elems:
                                           this_grid_elems + prev_grid_elems]
                this_leftmost_corners = [
                    this_anchors_out[1, :], this_anchors_out[4, :]]
                ...
                self.assertAllClose(
                    this_areas_out, expected_areas, rtol=0.1, atol=0.1)
                self.assertAllClose(this_leftmost_corners,
                                    expected_leftmost_corners)

...
void TrackedPersonsCache::initialize(rviz::Display *display, rviz::DisplayContext *context, ros::NodeHandle update_nh)
{
    ...
    m_tracked_person_subscriber = new rviz::AdditionalTopicSubscriber<tracking_msgs::TrackedPersons>("Tracked persons topic", display, context, update_nh,
...
void TrackedPersonsCache::processTrackedPersonsMessage(const tracking_msgs::TrackedPersons::ConstPtr &msg)
{
    ...
    m_cachedTrackedPersons.clear();
    foreach (tracking_msgs::TrackedPerson trackedPerson, msg->tracks)
    {

...
class TargetAssigner(object):
    ...
    def assign(self, anchors, groundtruth_boxes, groundtruth_labels=None,
        ...
        with tf.control_dependencies([shape_assert]):
            ...
            if num_anchors is not None:
                reg_targets = self._reset_target_shape(
                    reg_targets, num_anchors)
                cls_targets = self._reset_target_shape(
                    cls_targets, num_anchors)
                reg_weights = self._reset_target_shape(
                    reg_weights, num_anchors)
                cls_weights = self._reset_target_shape(
                    cls_weights, num_anchors)
    ...
    def _create_classification_weights(self,
        ...
        matched_indicator = tf.cast(
            match.matched_column_indicator(), tf.float32)
        ignore_indicator = tf.cast(
            match.ignored_column_indicator(), tf.float32)
...
def batch_assign_mask_targets(image_shape,
    ...
    for (groundtruth_masks, match, proposal_boxlist) in zip(
        ...
        def py_crop(np_masks, np_proposals, np_inds, height, width):
            mask_crops = np.zeros(
                (len(np_inds), height, width), dtype=np.float32)
            for i in range(len(np_inds)):
                roi = np_proposals[i, :]
                crop = np_masks[np_inds[i], int(roi[0]):int(
                    roi[2])+1, int(roi[1]):int(roi[3])+1]
        ...
        mask_crops = tf.py_func(
            py_crop,
            [groundtruth_masks, proposal_boxlist.get(), gt_inds_per_anchor,
             mask_height, mask_width],
            [tf.float32])
        mask_crops = tf.convert_to_tensor(
            tf.cast(mask_crops, tf.float32), name='mask_crops')

...
def drawMatches(img1, kp1, img2, kp2, matches, flags):
    ...
    out[:rows1, :cols1, :] = np.dstack(
        [img1, img1, img1]) if len(img1.shape) == 2 else img1

    # Place the next image to the right of it
    out[:rows2, cols1:(cols1 + cols2), :] = np.dstack([img2,
                                                       img2, img2]) if len(img2.shape) == 2 else img2
...
img2_kp = cv2.drawKeypoints(
    img2, kp2, flags=cv2.DRAW_MATCHES_FLAGS_DRAW_RICH_KEYPOINTS)

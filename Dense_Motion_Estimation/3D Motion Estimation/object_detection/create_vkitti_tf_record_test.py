...
with tf.Graph().as_default():
    ...
    with tf.device('/cpu:0'):
        ...
        flow_color = flow_to_color(tf.expand_dims(
            example['groundtruth_flow'], 0))[0, :, :, :]
        print(example.keys())
    ...
    with sess.as_default():
        for i in range(100):
            ...
            composed_flow_im.save(os.path.join(
                out_dir, str(img_id_np) + '_flow_from_motion.png'))
            flow_error_im.save(os.path.join(
                out_dir, str(img_id_np) + '_error.png'))
        sess.close()

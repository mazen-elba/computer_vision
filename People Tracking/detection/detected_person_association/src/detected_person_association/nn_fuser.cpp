...
void NearestNeighborFuserNodelet::onNewInputMessagesReceived(const std::vector<tracking_msgs::CompositeDetectedPersons::ConstPtr> &inputMsgs)
{
    ...
    const tracking_msgs::CompositeDetectedPersons::ConstPtr &firstSet = inputMsgs[0];
    ...
    else if (firstSet->elements.empty())
    {
        ...
        const tracking_msgs::CompositeDetectedPersons::ConstPtr &secondSet = inputMsgs[1];
    ...
    else
    {
        ...
        if (inputMsgs[1]->header.stamp > outputMsg->header.stamp)
            outputMsg->header.stamp = inputMsgs[1]->header.stamp;
        ...
        for (size_t d1Index = 0; d1Index < firstSet->elements.size(); d1Index++)
        {
            ...
            {
                const tracking_msgs::CompositeDetectedPerson &d1 = firstSet->elements[d1Index];
                const tracking_msgs::CompositeDetectedPerson &d2 = secondSet->elements[d2Index];
        ...
        for (;;)
        {
            ...
            const tracking_msgs::CompositeDetectedPerson &d1 = firstSet->elements[rowIndex];
            const tracking_msgs::CompositeDetectedPerson &d2 = secondSet->elements[colIndex];
            ...
            tracking_msgs::CompositeDetectedPerson fusedDetection;

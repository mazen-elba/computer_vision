...
void CompositeDetectedPersonsSynchronizer::initSynchronizer(const std::string &nodeletName, ros::NodeHandle nodeHandle, ros::NodeHandle privateNodeHandle, int minNumInputTopics, int maxNumInputTopics)
{
    ...
    foreach (std::string inputTopic, inputTopics)
    {
        ...
        if (!inputTopic.empty())
            m_subscribers.push_back(new SubscriberType(nodeHandle, inputTopic, 1));
    ...
    while (true)
    {
        ...
        if (activeTopics != m_previouslyActiveTopics)
        {
            ...
            else if (!m_publisher)
            {
                ...
                m_publisher.reset(new ros::Publisher(m_nodeHandle.advertise<tracking_msgs::CompositeDetectedPersons>("output", 5)));
            }
        }
...
void CompositeDetectedPersonsSynchronizer::handleNewInputMessages(const std::vector<tracking_msgs::CompositeDetectedPersons::ConstPtr> &inputMsgs)
{
...
void CompositeDetectedPersonsSynchronizer::onSingleInputMessageReceived(tracking_msgs::CompositeDetectedPersons::ConstPtr msg)
{
    std::vector<tracking_msgs::CompositeDetectedPersons::ConstPtr> msgs;
    msgs.push_back(msg);
    handleNewInputMessages(msgs);
}

void CompositeDetectedPersonsSynchronizer::onTwoInputMessagesReceived(tracking_msgs::CompositeDetectedPersons::ConstPtr msg1, tracking_msgs::CompositeDetectedPersons::ConstPtr msg2)
{
    std::vector<tracking_msgs::CompositeDetectedPersons::ConstPtr> msgs;
    msgs.push_back(msg1);
    msgs.push_back(msg2);

...
def jobFinished(request):
    ...
    numFinishedJobs += 1
    rospy.loginfo("Job has finished: %s, %d job(s) remaining!" %
                  (request.job_name, numTotalJobs - numFinishedJobs))
...
def main(argv=None):
    ...
    if 0 == numTotalJobs:
        ...
        rospy.loginfo("Will run until %d jobs have been completed." %
                      numTotalJobs)

        jobFinishedService = rospy.Service(
            'job_monitor', JobFinished, jobFinished)
        while numFinishedJobs < numTotalJobs and not rospy.is_shutdown():
            time.sleep(0.1)

        if numFinishedJobs < numTotalJobs:
            rospy.logwarn("Job monitor has been requested to terminate while %d job(s) were still active." % (
                numTotalJobs - numFinishedJobs))
        else:

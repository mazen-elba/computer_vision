...
def _write_tfrecord(record_dir, dataset_dir, split_name,
    ...
    for city_name in sorted(os.listdir(image_dir)):
        ...
        for i, filename in enumerate(files):
            ...
            next_file = "{}_{}_{}_leftImg8bit.png".format(
                city_name, seq, next_frame)
    ...
    for shard_id in range(num_shards):
        with tf.Graph().as_default(), tf.device('/cpu:0'):
            ...
            with tf.python_io.TFRecordWriter(record_filename) as tfrecord_writer:
                ...
                instance_img_ = _read_image(
                    shard_instance_paths, dtype=tf.uint16)
                disparity_img_ = _read_image(
                    shard_disparity_paths, dtype=tf.uint16)

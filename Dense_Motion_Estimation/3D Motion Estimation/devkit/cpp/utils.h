...
bool imageFormat(std::string file_name, png::color_type col, size_t depth, int32_t width, int32_t height)
{
    ...
    reader.read_info();
    if (reader.get_color_type() != col)
        return false;
    if (reader.get_bit_depth() != depth)
        return false;
    if (reader.get_width() != width)
        return false;
    if (reader.get_height() != height)
        return false;
    return true;
}
...
float statMin(std::vector<std::vector<float>> &errors, int32_t idx)
{
    float err_min = 1;
    for (int32_t i = 0; i < errors.size(); i++)
        if (errors[i][idx] < err_min)
            err_min = errors[i][idx];
    return err_min;
}
...
float statMax(std::vector<std::vector<float>> &errors, int32_t idx)
{
    float err_max = 0;
    for (int32_t i = 0; i < errors.size(); i++)
        if (errors[i][idx] > err_max)
            err_max = errors[i][idx];
    return err_max;
}

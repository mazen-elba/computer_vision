...
{
float PolarNNFuserNodelet::computeDistance(const tracking_msgs::CompositeDetectedPerson &d1, const tracking_msgs::CompositeDetectedPerson &d2)
{
    ...
    // Gating
    double angular_gating_distance = 0.5;
    getPrivateNodeHandle().getParamCached("angular_gating_distance", angular_gating_distance);
    if (angular_distance > angular_gating_distance)
        angular_distance = std::numeric_limits<float>::infinity();

    double radial_gating_distance = 2.5;
    getPrivateNodeHandle().getParamCached("radial_gating_distance", radial_gating_distance);
    if (radial_distance > radial_gating_distance)
        radial_distance = std::numeric_limits<float>::infinity();
    ...
    double angular_importance = 0.8;
    getPrivateNodeHandle().getParamCached("angular_importance", angular_importance);
    double radial_importance = 0.2;
    getPrivateNodeHandle().getParamCached("radial_importance", radial_importance);

    const float weightSum = angular_importance + radial_importance;
    angular_importance /= weightSum;
    radial_importance /= weightSum;
...
void PolarNNFuserNodelet::fusePoses(const tracking_msgs::CompositeDetectedPerson &d1, const tracking_msgs::CompositeDetectedPerson &d2, geometry_msgs::PoseWithCovariance &fusedPose)
{
    // Obtain and normalize weights for angular component
    double angular_weight1 = 0.5;
    getPrivateNodeHandle().getParamCached("fused_angular_weight_for_topic1", angular_weight1);
    double angular_weight2 = 0.5;
    getPrivateNodeHandle().getParamCached("fused_angular_weight_for_topic2", angular_weight2);

    const float angularWeightSum = angular_weight1 + angular_weight2;
    angular_weight1 /= angularWeightSum;
    angular_weight2 /= angularWeightSum;
    ...
    double radial_weight1 = 0.5;
    getPrivateNodeHandle().getParamCached("fused_radial_weight_for_topic1", radial_weight1);
    double radial_weight2 = 0.5;
    getPrivateNodeHandle().getParamCached("fused_radial_weight_for_topic2", radial_weight2);

    const float radialWeightSum = radial_weight1 + radial_weight2;
    radial_weight1 /= radialWeightSum;
    radial_weight2 /= radialWeightSum;

    // Obtain and normalize weights for orientation component (if at all relevant, most detections don't have an orientation...)
    double orientation_weight1 = 0.5;
    getPrivateNodeHandle().getParamCached("fused_orientation_weight_for_topic1", orientation_weight1);
    double orientation_weight2 = 0.5;
    getPrivateNodeHandle().getParamCached("fused_orientation_weight_for_topic2", orientation_weight2);

    const float orientationWeightSum = orientation_weight1 + orientation_weight2;
    orientation_weight1 /= orientationWeightSum;
    orientation_weight2 /= orientationWeightSum;
...
PLUGINLIB_DECLARE_CLASS(detected_person_association, PolarNNFuserNodelet, detected_person_association::PolarNNFuserNodelet, nodelet::Nodelet)

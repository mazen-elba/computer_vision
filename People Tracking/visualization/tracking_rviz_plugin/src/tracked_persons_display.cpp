...
void TrackedPersonsDisplay::processMessage(const tracking_msgs::TrackedPersons::ConstPtr &msg)
{
    ...
    for (vector<tracking_msgs::TrackedPerson>::const_iterator trackedPersonIt = msg->tracks.begin(); trackedPersonIt != msg->tracks.end(); ++trackedPersonIt)
    {
        ...
        {
            ROS_ERROR_STREAM("tracking_msgs::TrackedPersons contains duplicate track ID " << trackedPersonIt->track_id << "! Skipping duplicate track.");
            continue;
        }
        else
        {
...
PLUGINLIB_EXPORT_CLASS(tracking_rviz_plugin::TrackedPersonsDisplay, rviz::Display)

...
void ConvertToCompositeDetectionsNodelet::onInit()
{
    ...
    m_subscriber.reset(new ros::Subscriber(getNodeHandle().subscribe<tracking_msgs::DetectedPersons>("input", 2, &ConvertToCompositeDetectionsNodelet::onNewInputMessageReceived, this)));
...
{
    m_publisher.reset(new ros::Publisher(getNodeHandle().advertise<tracking_msgs::CompositeDetectedPersons>("output", 2)));
}
...
void ConvertToCompositeDetectionsNodelet::monitorInputTopic()
{
    ...
    while (true)
    {
        ...
        try
        {
            boost::this_thread::sleep(monitorInterval);
        }
...
void ConvertToCompositeDetectionsNodelet::onNewInputMessageReceived(const tracking_msgs::DetectedPersons::ConstPtr &inputMsg)
{
    ...
    foreach (const tracking_msgs::DetectedPerson &detectedPerson, inputMsg->detections)
    {
        tracking_msgs::CompositeDetectedPerson compositeDetectedPerson;
        ...
        else
        {
            ...
            for (int row = 3; row < 6; row++)
                for (int col = 3; col < 6; col++)
                    compositeDetectedPerson.pose.covariance[row * 6 + col] = 999999;
        }

        // Original detection
        tracking_msgs::DetectedPerson transformedDetectedPerson = detectedPerson;
...
PLUGINLIB_DECLARE_CLASS(detected_person_association, ConvertToCompositeDetectionsNodelet, detected_person_association::ConvertToCompositeDetectionsNodelet, nodelet::Nodelet)

...
void HumanAttributesDisplay::processMessage(const human_attribute_msgs::HumanAttributes::ConstPtr &msg)
{
    ...
    //
    foreach (const human_attribute_msgs::CategoricalAttribute &categoricalAttribute, msg->categoricalAttributes)
    {
        ...
        // Age group
        if (categoricalAttribute.type == human_attribute_msgs::CategoricalAttribute::AGE_GROUP)
        {
            ss.str("");
        ...
        // Gender
        else if (categoricalAttribute.type == human_attribute_msgs::CategoricalAttribute::GENDER)
        {
            ss.str("");
            ...
            if (valueWithHighestConfidence == human_attribute_msgs::CategoricalAttribute::GENDER_MALE)
                meshColor = Ogre::ColourValue(0, 1, 1, 1);
            if (valueWithHighestConfidence == human_attribute_msgs::CategoricalAttribute::GENDER_FEMALE)
    ...
    //
    foreach (const human_attribute_msgs::ScalarAttribute &scalarAttribute, msg->scalarAttributes)
    {
        // Check if there is already a visual for this particular track
        ...
        // Person height
        if (scalarAttribute.type == human_attribute_msgs::ScalarAttribute::PERSON_HEIGHT)
        {
            ss.str("");
...
PLUGINLIB_EXPORT_CLASS(tracking_rviz_plugin::HumanAttributesDisplay, rviz::Display)
